...
class CowrieUser(avatar.ConchUser):
    def __init__(self, username: bytes, server: server.CowrieServer, l_handler) -> None:
        avatar.ConchUser.__init__(self)
        self.learning_handler = l_handler

        # Learning episode START
        if CowrieConfig.getboolean('learning', 'reinforcement_mode'):
            self.learning_env = self.learning_handler.get_learning_env()
        else:
